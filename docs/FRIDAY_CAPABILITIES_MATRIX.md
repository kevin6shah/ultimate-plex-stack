...
| Pause for missing input and resume with `answer: ...` | `Promised` | `paused_for_input` heavy-task flow | Live-proven by Codex and live UX cleanup deployed | Login-wall/sign-up-specific gating is still a separate backlog item | Extend the same substrate into login/account-gated flows |
